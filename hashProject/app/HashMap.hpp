...
//
// MODIFIED FILE by AARON ZHONG ID: 67737879
...
public:
  ...
  // .|. QUESTION: How to write it in the CPP?
...
public:
  ...
  //...Print current key-pair-index values in the hashArray
  void printHashArray();
  //...CLEAR entire hashArray (remove all nodes)
  void clearHashArray();
...
private:
  ...
  */
  //................................. .\. Below is code written by 67737879
  ...
  /*........... SIDE NOTE.....................
    ...
    hashArray is synonymous (how I defined it) as hashTable
  ...
  */
  ...
  //....A static double that's set to the maximumLoadSize of 0.8
  static constexpr double maxLoadPercentage = 0.8;

  //....Take a hashArray and copy the contents (NOT pointers) into THIS hashArray
  void hashArrayCopy( Node** copyFromHere);

  //...Add the key/pair to the next this->next that is 
  //...not an actual Node struct object
  void addIfNodeNull(const std::string& key, const std::string& value);

  /*....reHash the entire hashArray in THIS
       by:
        - creating a new hashArray using the old
        - delete the old  
  */
  void reHashEntireHashArray();

  //...Calculate the hashArrayIndex value (which comes from the hashFunction & size)
  const unsigned int calculateHashArrayIndex(const std::string& keyString) const;
  ...
  //...hashArray AKA hashTable
  Node** hashArray;

  //....Other variables.......
  double totalNumberOfPairs;
  unsigned int currentHashArraySize;
};
